...
В этой статье рассматривается конвейер, реализованный в [`extract.py`](extract.py), который объединяет три взаимодополняющих подхода:
...
Оба метода дают пересекающиеся, но дополняющие друг друга результаты. В [`clean_utils.py`](clean_utils.py) они объединяются и очищаются:
